...
你可以通过 `ActionProviders.registerActionProvider` 来给额外图标的`点击执行操作`功能增加更多你自己定义的类型，当前自带以下类型，实现方式详见 [actions](https://github.com/MrXiaoM/PluginBase/tree/main/modules/actions/src/main/java/top/mrxiaom/pluginbase/actions) 包中自带的操作。
...
+ `[broadcast]当前子服公告消息`
+ `[title]标题\n副标题` 或者 `[title]副标题`
+ `[sound]音效` 或者 `[title]音效,音量,音调`
+ `[close]` (关闭界面)

特殊地，`[title]`、`[message]`、`[broadcast]` 可选开启 Adventure 支持，如果开启，将支持使用 MiniMessage 格式。

音效使用原版格式，例如 `block.note_block.harp`。

开发者如果需要添加类型，请记得要通过 `PAPI.setPlaceholders(player, s)` 来处理 PlaceholderAPI 变量。
