...
permalink: /elopers/base/language
---
...
public class Messages {
    public static final Message arguments__not_a_number = field("请输入一个数字");
    public static final Message arguments__internal = field("出现了一个内部错误 (%s)，请联系服务器管理员");
    @Language("player.not-online") // 如果你乐意，还可以像这样指定配置键
    public static final Message player__not_online = field("玩家 %player% 不在线 (或不存在)");
    public static final Message list = field("可以用多个参数", "代表list");
    public static final Message list_2 = field(Lists.newArrayList("用 List", "也是可以的"));
}
```

生成的配置与枚举类基本相同，仅仅是 `errors` 变为了 `messages`。
