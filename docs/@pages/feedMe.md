---
title: 赞助作者
sidebar: false
article: false
permalink: /feedme
---

![header](https://static.mcio.dev/legacy/6a193100c17c6e53b6587213.webp)

你的支持是我（人间工作）继续更新插件的动力！

相信同为 Minecraft 服务器内容创作者的你也知道，插件开发并非一件容易的事，如何端好服务端兼容这碗水更是件困难的事。  
方便起见，我的所有公开插件**都开放了源代码**，欢迎各位同行参考使用。

虽然文档站部署在大善人 Cloudflare Pages 上面，但无数次给予我灵感的 Minecraft 服务器实例、该站点的域名、以及无数个日夜的更新，都需要**实打实的成本**。

如果你从中受益，愿意**请我喝杯奶茶/咖啡**、支持我继续深耕，欢迎通过以下赞助方式给予我物质支持。

你的赞助将会用于：
- Minecraft 服务器实例的维护支出
- 该文档站点的维护支出
- 确保人类实体的存活，持续跟进插件更新

因为有你，免费的优质内容才能一直存在。

衷心感谢每一位支持者。

## 中国大陆赞助方式

通过 **微信/支付宝** 对我进行直接支持。

![二维码](https://static.mcio.dev/mrxiaom.top/images/feed.webp)

通过 **爱发电** 对我进行支持。

<iframe src="https://ifdian.net/leaflet?slug=mrxiaom" width="100%" scrolling="no" height="200" frameborder="0"></iframe>

或者通过[雨云](https://www.rainyun.com/uwu_)优惠码`uwu`对我进行间接支持。

## 海外赞助方式

前往 [Paypal](https://www.paypal.com/paypalme/mrxiaom) 向 陈苼 付款。  
或者购买 [Rewarble](https://skine.com/en-cn/rewarble?utm_source=rewarble.com) 礼品卡发送到邮箱 `coolxiaom95@gmail.com`。
