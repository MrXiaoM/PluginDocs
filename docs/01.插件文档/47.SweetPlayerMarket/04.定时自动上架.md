---
title: 定时自动上架
date: 2026-01-01 13:28:16
permalink: /docs/playermarket/auto-deploy
---
...
自动上架配置文件是 `auto-deploy.yml`，每个子服都有，但**只需要配置一个子服即可**。定时器的持久化数据保存在本地 `auto-deploy.data.yml` 文件而不上传数据库，**没有必要使用数据库**储存、同步这些数据。
