---
title: 命令与权限
date: 2025-09-01 17:12:56
permalink: /docs/playermarket/commands
sponsorship: true
---

这个页面介绍 SweetPlayerMarket 的命令与权限。

## 命令

根命令为 `/sweetplayermarket`，别名为 `/playermarket`, `/spm`, `/pm`  
以 `<>` 包裹的为必选参数，以 `[]` 包裹的为可选参数。

玩家命令
| 命令 | 描述 | 权限 |
| --- | --- | --- |
| `/pm open` | 打开全球市场首页 | `sweet.playermarket.open` |
| `/pm me` | 查看自己的已上架商品 | `sweet.playermarket.me` |
| `/pm tags` | 打开选择分类菜单 | `sweet.playermarket.tags` |
| `/pm create <商店类型> <价格> <货币> [单个数量] [总份数]` | 上架商品到全球市场 | `sweet.playermarket.create` |
| `/pm create <商店类型> --menu` | 打开上架菜单 | `sweet.playermarket.create` |

管理员命令
| 命令 | 描述 | 权限 |
| --- | --- | --- |
| `/pm open [玩家]` | 为自己或某人打开全球市场首页 | `sweet.playermarket.open.other` |
| `/pm me [玩家]` | 为自己或某人查看已上架商品 | `sweet.playermarket.open.other` |
| `/pm tags [玩家]` | 为自己或某人打开选择分类菜单 | `sweet.playermarket.tags.other` |
| `/pm recalc tags` | 重新计算所有商品的标签 | `sweet.playermarket.recalc` |
| `/pm recalc index` | 重新计算所有商品的搜索索引 | `sweet.playermarket.recalc` |
| `/pm server [玩家]` | 查看特殊账户“服务器”的已上架商品，通常是自动上架的商品 | `sweet.playermarket.server` |
| `/pm auto-deploy <配置名> <操作>` | 自动上架功能的测试命令 | `sweet.playermarket.auto-deploy` |
| `/pm reload assets` | 重载语言文件配置，如果不存在将会自动下载 | OP/控制台 |
| `/pm reload database` | 重新连接数据库 | OP/控制台 |
| `/pm reload` | 重载插件配置文件 | OP/控制台 |

`open` 命令后面可以添加以下选项
+ `--type=类型` 筛选商品类型
+ `--currency=类型` 筛选货币类型
+ `--tag=标签名` 筛选指定标签

`me` 命令后面可以添加以下选项
+ `--notice` 仅显示有 `notice_flag` 的商品
+ `--only-out-of-stock` 仅显示已经下架的商品

`create` 命令（不管是手动上架还是打开菜单）可以添加以下选项
+ `--system=自定义名称` 与自动上架类似，以系统商店的身份手动上架商品（需要权限 `sweet.playermarket.create.system`）

## 权限

以下为本插件可用的其它权限
+ `sweet.playermarket.create.currency.vault` 允许使用 Vault 金币上架商品
+ `sweet.playermarket.create.currency.playerpoints` 允许使用 PlayerPoints 点券上架商品
+ `sweet.playermarket.create.currency.mpoints.<点数>` 允许使用 MPoints 点数上架商品
+ `sweet.playermarket.create.currency.coinsengine.<货币>` 允许使用 CoinsEngine 货币上架商品
+ `sweet.playermarket.create.currency.custom.<货币>` 允许使用 currencies.yml 中的自定义货币上架商品
+ `sweet.playermarket.create.bypass.cost` 绕过创建商品手续费限制
+ `sweet.playermarket.create.bypass.currency` 绕过创建商品货币类型限制
+ `sweet.playermarket.create.bypass.type` 绕过创建商品类型（出售、收购）限制
+ `sweet.playermarket.admin` 全球市场管理员权限，暂用于下架其他玩家的商品
