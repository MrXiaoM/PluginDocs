...
| `/checkout points <类型> <金额> --mock` | 模拟下单指定金额的点券，立即获得点券 | `sweet.checkout.points` |
| `/checkout buy <商品ID> <类型> --mock` | 模拟下单指定商品，立即获得商品 | 在商品配置定义 |
...
特殊地，`--mock` 参数需要权限 `sweet.checkout.mock` 才能使用，该参数仅供测试使用。
